...
export default function DiseaseCard({
  ...
  return (
    <Link
      href={`/disease/${id}`}
      className={`group block rounded-2xl border p-4 sm:p-5 transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 ${
        isWarning
    ...
    >
      <div className="flex items-start justify-between gap-2 sm:gap-3 mb-2.5 sm:mb-3">
        <h3 className="text-base sm:text-lg font-bold text-slate-800 group-hover:text-blue-600 transition-colors capitalize">
          {name.replace(/_/g, " ")}
        </h3>
      ...
      <div className="flex flex-wrap gap-1 sm:gap-1.5 mb-3 sm:mb-4">
        {matched_symptoms.slice(0, 6).map((symptom) => (
          <span
            key={symptom}
            className="inline-flex items-center px-2 sm:px-2.5 py-0.5 rounded-lg text-xs font-medium bg-blue-50/80 text-blue-700 border border-blue-200/60 capitalize"
          >
            {symptom.replace(/_/g, " ")}
          </span>
        ))}
      ...
      {/* Score bar */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 sm:gap-3 pt-2.5 border-t border-slate-100">
        <div className="w-full sm:flex-1 h-1.5 sm:h-2 bg-slate-100 rounded-full overflow-hidden">
          <div
